...
| `/phi (sign\|签到)` | 签到获取Notes |
| `/phi (task\|我的任务)` | 查看自己的任务 |
| `/phi (retask\|刷新任务)` | 刷新任务，需要花费20Notes |
...
| 功能名称 | 功能说明
| :- | :-
| `/phi backup`<!-- (back)?--> | 备份存档文件<!--，+ back 发送该备份文件-->，自动保存在 `data/phi-plugin/backup` 目录下 |
| `/phi restore` | 从备份中还原，不会丢失已有数据，需要将文件放在 `data/phi-plugin/backup` 目录下 |
