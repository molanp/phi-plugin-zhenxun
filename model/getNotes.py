...
class getNotes:
    @classmethod
    async def getPluginData(cls, user_id: str):
        """
        获取uid对应的娱乐数据
    ...
    @classmethod
    async def putPluginData(cls, user_id: str, data: dict):
        """保存 user_id 对应的娱乐数据"""
        session = await getSave.get_user_token(user_id)
        if data.get("rks") is not None:
            ...
            await readFile.SetFile(savePath / session / "history.json", history)
        await readFile.SetFile(pluginDataPath / f"{user_id}_.json", data)

    @classmethod
    async def getNotesData(cls, user_id: str):
        """
        获取并初始化用户数据
    ...
    @classmethod
    async def putMoneyData(cls, user_id: str, data: dict):
        """保存用户数据"""
