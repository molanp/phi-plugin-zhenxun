    ...
    async def init(cls):
        ...
        if nick:
            ...
            if flag:
                ...
                logger.success("[phi-plugin] 自动修正别名", "phi-plugin")
        ...
        # SP信息
        cls.sp_info = await readFile.FileReader(infoPath / "spinfo.json")

        for key, value in cls.sp_info.items():
            value["sp_vis"] = True
            if value.get("illustration_big"):
                cls.illlist.append(value["song"])
        
        # TODO: 完善其他配置，忙了一上午该玩游戏了
